...
func newVXLANDevice(devAttrs *vxlanDeviceAttrs) (*vxlanDevice, error) {
	link := &netlink.Vxlan{
		LinkAttrs: netlink.LinkAttrs{
			Name:         devAttrs.name,
			HardwareAddr: VTEPForNode(os.Getenv("NODENAME")),
		},
...
// Calculates a deterministic VTEP (MAC address) for the
// VXLAN device on the given node.
func VTEPForNode(nodename string) net.HardwareAddr {
	hasher := sha1.New()
	hasher.Write([]byte(nodename))
	sha := hasher.Sum(nil)
	var hw net.HardwareAddr
	hw = net.HardwareAddr(append([]byte("f"), sha[0:5]...))
	log.Infof("Calculated MAC: %s->%s", nodename, hw)
	return hw
}

func PublicIPForNode(nodename string) ip.IP4 {
	_, cli := calicoclient.CreateClient()
	log.Infof("Getting public IP for node: %s", nodename)
	n, err := cli.Nodes().Get(context.Background(), nodename, options.GetOptions{})
	if err != nil {
		log.Fatalf("Failed to get node: %s", err)
	}
	nodeip, _, err := net.ParseCIDR(n.Spec.BGP.IPv4Address)
	if err != nil {
		log.Fatalf("Failed to parse IP: %s", err)
	}
	log.Infof("Got public IP for node: %s", nodename)
	return ip.FromIP(nodeip)
}
